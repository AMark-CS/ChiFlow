...
from ..common.geometry import construct_3d_basis, global_to_local, get_backbone_dihedral_angles
from ..common.layers import AngularEncoding
from ..common.constants import BBHeavyAtom, AA
...
class ResidueEmbedding(nn.Module):
    ...
    def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, fragment_type, structure_mask=None, sequence_mask=None):
        ...
        """
        N, L = aa.size()
        residue_mask = mask_atoms[:, :, BBHeavyAtom.CA] # (N, L)
        ...
        bb_dihedral, mask_bb_dihed = get_backbone_dihedral_angles(pos_atoms, chain_nb=chain_nb, res_nb=res_nb, mask=residue_mask)
        ...
        out_feat = out_feat * residue_mask[:, :, None]
        return out_feat
...
class PerResidueEncoder(nn.Module):
    ...
    def forward(self, aa, phi, phi_mask, psi, psi_mask, chi, chi_mask, residue_mask):
        """
        Args:
            aa: (N, L)
            phi, phi_mask: (N, L)
